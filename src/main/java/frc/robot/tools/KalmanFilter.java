...
public class KalmanFilter<States extends Num, Inputs extends Num, Outputs extends Num> {
  ...
  public KalmanFilter(
    ...
    // S = CPCᵀ + R
    var S = C.times(m_P).times(C.transpose()).plus(discR);
   ...
   * @param dtSeconds Time since last correction
   */
  @SuppressWarnings("ParameterName")
  public void correct(Matrix<Inputs, N1> u, Matrix<Outputs, N1> y, double dtSeconds) {
    ...
    m_P =
        Matrix.eye(m_states)
            .minus(m_K.times(C))
            .times(m_P)
            .times(Matrix.eye(m_states).minus(m_K.times(C)).transpose())
            .plus(m_K.times(discR).times(m_K.transpose()));
  }
}